...
pair<unordered_set<int>,unordered_set<int>> semiGreedy(unordered_map<int, vector<Edge>> &adjList, const vector<Edge>& edges, double alpha, int n)
{
    ...
    cout<<"semi: "<<getCut(cutX,cutY,edges)<<endl;
...
int main()
{
    ...
    //Simple Randomized
    pair<unordered_set<int>,unordered_set<int>> solution = semiGreedy(adjList,edges, 0,n); //alpha=0
    ...
    cout<<sol.first<<","<<getCut(sol.second.first,sol.second.second, edges)<<",";

    //GRASP
    int iterations = 3; // Number of GRASP iterations
